	...
	if(type == FOMODINSTALLER)
	{
		...
		for(auto iter = fomod_files.begin(); iter != fomod_files.end(); iter++)
		{
			...
			if(sfs::is_directory(tmp_dir / source_file))
			{
				...
				{
					pu::moveFilesToDirectory(tmp_dir / source_file, destination / dest_file, contains_no_duplicates);
				}
				else
				{
					pu::copyOrMoveFiles(tmp_dir / source_file, destination / dest_file, contains_no_duplicates);
				}
			}
			else
			{
				...
				{
					pu::copyOrMoveFiles(tmp_dir / source_file, destination / source_file.filename(), contains_no_duplicates);
				}
				else
				{
					pu::copyOrMoveFiles(tmp_dir / source_file, destination / dest_file, contains_no_duplicates);
