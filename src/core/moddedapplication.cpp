	...
	name_(name), staging_dir_(staging_dir), command_(command), icon_path_(icon_path)
{
	...
	sfs::copy(staging_dir_ / CONFIG_FILE_NAME, staging_dir_ / ("." + CONFIG_FILE_NAME + ".bak"), sfs::copy_options::overwrite_existing);
}
...
void ModdedApplication::deployModsFor(std::vector<int> deployers)
{
	str::sort(deployers,
		...
		{
			return this->deployers_[depl_l]->getDeployPriority() < this->deployers_[depl_r]->getDeployPriority();
		}
	);
	...
	for(auto [i, deployer] : str::enumerate_view(deployers))
	{
		...
		if(!deployers_[deployer]->isAutonomous())
		{
			...
			{
				auto mod_iter = str::find_if(
					installed_mods_, [id = mod_id](const Mod& m)
					{
						return m.id == id;
					}
				);
	...
	const std::string& installer_type)
{
	...
	for(int mod_id : mod_ids)
	{
		...
		auto mod_iter = std::find_if(installed_mods_.begin(), installed_mods_.end(), [mod_id](Mod m) { return m.id == mod_id; });
		if(mod_iter == installed_mods_.end())
		{
			continue;
		}
...
void ModdedApplication::changeModName(int mod_id, const std::string& new_name)
{
	auto iter = std::find_if(
		installed_mods_.begin(), installed_mods_.end(), [mod_id](Mod m)
			{
				return m.id == mod_id;
			}
	);
		...
		for(int depl = 0; depl < deployers_.size(); depl++)
		{
			...
			for(int prof = 0; prof < profile_names_.size(); prof++)
			{
				...
				auto iter = str::find_if(
					loadorder, [mod_id](const auto& tuple)
					{
						return std::get<0>(tuple) == mod_id;
					}
				);
	...
	const std::vector<bool>& deployers)
{
	...
	for(int i = 0; i < mod_ids.size(); i++)
	{
		...
		for(int depl = 0; depl < deployers.size(); depl++)
		{
			...
			{
				removeModFromDeployer(depl, mod_id, is_last_mod, is_last_mod ? &node.child(depl) : dummy_node);
			}
		}
	}
}
...
DeployerInfo ModdedApplication::getDeployerInfo(int deployer)
{
	if(!(deployers_[deployer]->isAutonomous()))
	{
		...
		for(const auto& [id, e] : loadorder)
		{
			mod_names.push_back(
				std::ranges::find_if(
					installed_mods_, [id = id](auto& mod)
					{
						return mod.id == id;
					}
				)->name);
	...
	else
	{
		...
		return {
			deployers_[deployer]->getModNames(),
...
void ModdedApplication::cleanupFailedInstallation()
{
	...
	auto iter = std::find_if(
		installed_mods_.begin(),
		installed_mods_.end(),
		[this](const Mod& m)
		{
			return m.id == this->last_mod_id_;
		}
	);
...
void ModdedApplication::addManualTag(const std::string& tag_name)
{
	...
	{
		throw std::runtime_error(std::format("Error: A tag with the name '{}' already exists.", tag_name));
	}
...
void ModdedApplication::addAutoTag(const Json::Value& json_tag, bool update)
{
	...
	{
		throw std::runtime_error(std::format("Error: A tag with the name '{}' already exists.", json_tag["name"].asString()));
	}

	auto_tags_.emplace_back(json_tag);
	auto select_id = [](const auto& mod)
	{
		return mod.id;
	};
...
void ModdedApplication::editAutoTags(const std::vector<EditAutoTagAction>& actions)
{
	...
	try
	{
		...
		for(const auto& action : actions)
		{
			...
			{
				changeAutoTagEvaluator(action.getName(), action.getExpression(), action.getConditions(), false);
...
void ModdedApplication::updateAutoTags(const std::vector<int> mod_ids)
{
	...
	node.addChildren(
		{
			1.0f,
			std::max(1.0f, 8.0f * (float)mod_ids.size() / (float)installed_mods_.size())
		}
	);
...
void ModdedApplication::suppressUpdateNotification(const std::vector<int>& mod_ids)
{
	for(int mod_id : mod_ids)
	{
		...
		{
			iter->suppress_update_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
		}
	}
	updateSettings(true);
}
	...
	const std::vector<std::string>& auto_tags)
{
	...
	for(int deployer_id : deployers)
	{
		...
		json["deployers"][i]["deploy_mode"] = deployer->getDeployMode() == Deployer::copy ? "copy" : "hard_link";
...
void ModdedApplication::updateSettings(bool write)
{
	...
	for(int depl = 0; depl < deployers_.size(); depl++)
	{
		...
		json_settings_["deployers"][depl]["enable_unsafe_sorting"] = deployers_[depl]->getEnableUnsafeSorting();
		...
		if(!deployers_[depl]->isAutonomous())
		{
			for(int prof = 0; prof < profile_names_.size(); prof++)
			{
				...
				{
					json_settings_["deployers"][depl]["profiles"][prof]["loadorder"][mod]["id"] = std::get<0>(loadorder[mod]);
					json_settings_["deployers"][depl]["profiles"][prof]["loadorder"][mod]["enabled"] = std::get<1>(loadorder[mod]);
				}
				...
				for(int group = 0; group < conflict_groups.size(); group++)
				{
					...
					{
						json_settings_["deployers"][depl]["profiles"][prof]["conflict_groups"][group][i] = conflict_groups[group][i];
					}
				}
			}
		}
...
void ModdedApplication::updateState(bool read)
{
	...
	for(int i = 0; i < installed_mods.size(); i++)
	{
		...
		{
			throw ParseError("Unknown installer type: " + installer + " in \"" + (staging_dir_ / CONFIG_FILE_NAME).string() + "\"");
		}
	...
	for(int group = 0; group < groups.size(); group++)
	{
		...
		if(std::find(groups_[group].begin(), groups_[group].end(), active_member) == groups_[group].end()
	...
	for(int depl = 0; depl < deployers.size(); depl++)
	{
		...
		{
			throw ParseError("Unknown deployer type: " + type + " in \"" + (staging_dir_ / CONFIG_FILE_NAME).string() + "\"");
		}

		Deployer::DeployMode deploy_mode = Deployer::hard_link;
		if(deployers[depl].isMember("use_copy_deployment"))
		{
			deploy_mode = deployers[depl]["use_copy_deployment"].asBool() ? Deployer::copy : Deployer::hard_link;
		}
		else
		{
	...
	if(json_settings_.isMember("manual_tags"))
	{
		for(auto& tag_entry : json_settings_["manual_tags"])
		{
			...
			{
				throw ParseError(std::format("Manual tag \"{}\" found more than once.", tag_entry["name"].asString()));
			}
	...
	if(json_settings_.isMember("auto_tags"))
	{
		for(auto& tag_entry : json_settings_["auto_tags"])
		{
			...
			{
				throw ParseError(std::format("Auto tag \"{}\" found more than once.", tag_entry["name"].asString()));
			}
...
void ModdedApplication::splitMod(int mod_id, int deployer)
{
	...
	{
		const auto mod_dir_optional = pu::pathExists(
			dir,
			staging_dir_ / std::to_string(mod_id),
...
void ModdedApplication::replaceMod(const ImportModInfo& info)
{
	...
	auto index = str::find_if(installed_mods_, [group = info.target_group_id](const Mod& m) { return m.id == group; });
	...
	while(pu::exists(staging_dir_ / std::to_string(mod_id)) && mod_id < std::numeric_limits<int>().max())
	{
		mod_id++;
	}
	...
	const sfs::path tmp_replace_dir = staging_dir_ / (std::string("tmp_replace_") + std::to_string(mod_id));

	const auto mod_size = Installer::install(
		info.current_path,
		tmp_replace_dir,
		info.installer_flags,
		info.installer,
...
void ModdedApplication::performUpdateCheck(const std::vector<int>& target_mod_indices)
{
	...
	{
		installed_mods_[i].remote_update_time = nexus::Api::getNexusPage(installed_mods_[i].remote_source).mod.updated_time;
...
std::string ModdedApplication::generalizeSteamPath(const std::string& path)
{
	...
	std::regex prefix_regex(R"((\/.*\/steamapps\/compatdata\/\d+\/pfx\/(?:drive_c|dosdevices\/c:))(?:\/.*)?)");
