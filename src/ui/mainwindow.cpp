...
void MainWindow::updateOutdatedSettings()
{
	...
	if(versionIsLessOrEqual(previous_app_version_, "1.0.4"))
	{
		const std::map<std::string, std::string> old_urls = {
			...
			{ "starfield_url", "https://raw.githubusercontent.com/loot/starfield/master/masterlist.yaml" },
...
void MainWindow::onModListContextMenu(QPoint pos)
{
	...
	else
	{
		...
		ui->actionRemove_Other_Versions->setVisible(idx.data(ModListModel::mod_group_role).toInt() > -1);
		...
		ui->actionReinstall_From_Local->setVisible(!local_path.empty() && std::filesystem::exists(local_path));
...
void MainWindow::onGetAppInfo(AppInfo app_info)
{
	...
	for(int i = 0; i < app_info.deployers.size(); i++)
	{
		...
		ui->info_deployer_list->setItem(i, 3, new QTableWidgetItem(QString::number(app_info.deployer_mods[i])));
...
void MainWindow::onExtractionComplete(ImportModInfo info)
{
	...
	if(was_successful)
	{
		...
		onReceiveError("Error", ("Failed to import mod from \"" + info.local_source.string() + "\"").c_str());
}
