...
void SettingsDialog::init()
{
	...
	ui->log_level_box->setCurrentIndex(settings.value("log_level", Log::LogLevel::LOG_INFO).toInt() - 2);
	...
	ui->show_error_cb->setCheckState(settings.value("log_on_error", true).toBool() ? Qt::Checked : Qt::Unchecked);
	...
	ui->valid_api_key_label->setText(key_is_valid ? "Nexus API key is valid." : "No valid API key set");
...
void SettingsDialog::on_set_api_key_button_clicked()
{
	...
	ui->premium_user_label->setText(user_info->second ? "Account is premium" : "Account is NOT premium");
